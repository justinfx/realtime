...
func main() {
	...
	if c, err := getConf(); err == nil {
		if v, e := c.Bool("Server", "debug"); e == nil {CONFIG.DEBUG = v}
		if v, e := c.Int("Server", "websocket-port"); e == nil {CONFIG.PORT = v}
		if v, e := c.String("Server", "allowed-domain"); e == nil {domainVal = v}
		if v, e := c.Int("Messaging", "message-cache-limit"); e == nil {CONFIG.HWM = v}
		...
		if v, e := c.String("Server", "allowed-types"); e == nil && v != "" {
			...
			if len(types) != 0 { CONFIG.ALLOWED_TYPES = types }			
		}
	}
	...
	if *fDebug { CONFIG.DEBUG = true }
	if *fPort > 0 { CONFIG.PORT = *fPort }
	if *fDomains != "" { domainVal = *fDomains }
	...
	if len(CONFIG.ALLOWED_TYPES) > 0 {
		...
		for i, t := range CONFIG.ALLOWED_TYPES {
			switch t {
			case "xhr-polling": 	config.Transports[i] = socketio.NewXHRPollingTransport(10e9, 5e9)
			case "xhr-multipart": 	config.Transports[i] = socketio.NewXHRMultipartTransport(0, 5e9)
			case "websocket": 		config.Transports[i] = socketio.NewWebsocketTransport(0, 5e9)
			case "htmlfile": 		config.Transports[i] = socketio.NewHTMLFileTransport(0, 5e9)
			case "flashsocket": 	config.Transports[i] = socketio.NewFlashsocketTransport(0, 5e9)
			case "json-polling": 	config.Transports[i] = socketio.NewJSONPPollingTransport(0, 5e9)
			}
		}
	}
